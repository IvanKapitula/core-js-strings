...
function removeLastOccurrences(str, value) {
  ...
  if (index !== -1) {
    ...
    return str
      .substring(0, index)
      .concat(str.substring(index + value.length + 1, str.length - 1));
  }
  return str;
}
...
function findLongestWord(sentence) {
  const mass = sentence.split(' ').reverse();
  let res = '0';
  let res1 = '';
  for (let i = 0; i < mass.length - 1; i += 1) {
    res = String(Math.max(mass[i].length, +res));
  }
  for (let i = 0; i < mass.length; i += 1) {
    if (mass[i].length === +res) {
      res1 = mass[i];
    }
  }
  return res1;
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (arg) => cipherAlphabet[alphabet.indexOf(arg)]);
}
